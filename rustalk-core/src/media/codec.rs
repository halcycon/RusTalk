...
pub fn standard_codecs() -> Vec<Codec> {
    vec![
        ...
        Codec::new(
            ...
            8000,
            1,
            "Internet Low Bitrate Codec (13.33/15.2 kbps)",
            true,
        ),
        ...
        Codec::new(
            ...
            8000,
            1,
            "Adaptive Multi-Rate narrowband (4.75-12.2 kbps)",
            true,
        ),
        Codec::new(
            "AMR-WB",
            ...
            16000,
            1,
            "Adaptive Multi-Rate wideband (6.6-23.85 kbps)",
            true,
        ),
...
impl CodecConfig {
    ...
    pub fn add_codec(&mut self, mut codec: Codec) -> Result<(), String> {
        ...
        if self
            .codecs
            .iter()
            .any(|c| c.payload_type == codec.payload_type)
        {
            return Err(format!(
                "Codec with payload type {} already exists",
                codec.payload_type
            ));
        }
    ...
    pub fn sort_by_priority(&mut self) {
        self.codecs.sort_by(|a, b| match (a.priority, b.priority) {
    ...
    pub fn get_priority_ordered(&self) -> Vec<&Codec> {
        ...
        enabled.sort_by(|a, b| match (a.priority, b.priority) {
            (Some(pa), Some(pb)) => pa.cmp(&pb),
            (Some(_), None) => std::cmp::Ordering::Less,
            (None, Some(_)) => std::cmp::Ordering::Greater,
            (None, None) => std::cmp::Ordering::Equal,
